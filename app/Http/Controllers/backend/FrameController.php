...
class FrameController extends Controller
{
    ...
    public function editFramepost(Request $req){
        ...
        if($frame){
          ...
          if($req->hasFile('file_upload')){
            $framefile= Frame::get();
            $kt =0;
            foreach ($framefile as $ff) {
              if (($ff->file == $frame->file) && ($ff->id != $frame->id) ){
                  $kt =$kt+1;
              }
            }
            if ($kt==0){
              if(File::exists(public_path().$frame->file)){
                  File::delete(public_path().$frame->file);
              }
            }
                      ...
                      foreach($arr_attrID as $key => $id_attr){
                         ...
                         if(sizeof($attr_obj)){
                            ...
                            if($name =="Giá") {
                              ...
                              // nếu không có thuộc tính nào bằng gí thì tạo mới
                              if(!$skjau){
                                $skjau = new Attribute;
                                $skjau->name = "Giá";
    ...
    public function del_frame_post(Request $req){
        ...
        if($frame){
            $comment = CommentFrame::where('frame_id',$frame->id)->delete();
            $frame_cate = FramCate::where('frame_id',$frame->id)->delete();
