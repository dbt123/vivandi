...
class PostController extends Controller
{
    ...
    public function post_create_post(Request $req){
    	if (!$req->post_title){
        return redirect()->route('posts.add')->with('error','Bạn chưa nhập tiêu đề');
	    }
	    if (!$req->hasFile('post_img')){
	        return redirect()->route('posts.add')->with('error','Bạn chưa chọn ảnh đại diện bài viết');
	    }
    ...
    public function del_post(Request $req){
        ...
        $tag_post = Tag_post::where('post_id',$post->id)->delete();
        $post->delete();
        echo 'true';
    }
