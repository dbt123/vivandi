...
{   
    ...
    public function listProduct(){
        $attribute_count = FrameAttribute::select("frame_attributes.attribute_id",'frame_attributes.id as xyz','frame_attributes.product_id','frame_attributes.frame_id','frame_attributes.status_frame','attributes.*',DB::raw('count(frame_attributes.attribute_id) as num'))->groupby('frame_attributes.attribute_id','frame_attributes.status_frame')->leftjoin('attributes','attributes.id','=','frame_attributes.attribute_id')->orderby('attributes.name')->get();
          ...
          if($value->status_frame == 0){
            ...
            foreach ($attribute_count_temp as $key_temp => $value_temp) {
                if($value->name == $value_temp->name && $value->value == $value_temp->value  && $value->xyz != $value_temp->xyz && $value_temp->status_frame == 1){
                  $c++;
                  $attribute_count[$key]->num_hide =  $x;
                  $attribute_count[$key]->num = 0;
                  $attribute_count[$key_temp]['num_hide'] =  $x;
                }
            }  
            // neu khong co thang nao public giong no
            if($c == 0 ){
              $attribute_count[$key]->num_hide = $value->num;
              $attribute_count[$key]->num = 0;
            }else{
              // neu co public thi xoa thang an di
              unset($attribute_count[$key]);
            }
          }
        }
        $i = 0;
        foreach ($attribute_count as $key => $value) {
            if(!isset($attribute_count[$i])){
                $attribute_count[$i] = $attribute_count[$key];
                unset($attribute_count[$key]);
            }
            $i++;
        }
    ...
    public function formProduct(Request $req){
      ...
      if($product->save()){
              ...
              $frame->assurance = $assurance;

              // kiểm tra file upload
              if($req->hasFile('file_upload')){
                $file = array('file' => Input::file('file_upload'));
                $rules = array('file' => 'max:20480');
                ...
                $file->move(public_path().'/assets/file/', $nameimg);
                $frame->file = $nameimg;
                }
              }
    ...
    public function getPhanTrangNhapKho(Request $req){
      ...
      $frame = Frame::find($id);
      if($frame){
        $email_out_of_stocks = Email_out_of_stocks::where('add_1',$frame->id)->paginate(5);
 ...
 // phí vận chuyển
  ...
  public function AgencyCustomer(Request $req){
    $id = $req->id;
    $search =trim($req->cus);
    ...
    $order = Order::whereIn('id',$in)->select('phone', DB::raw('count(*) as total'))->selectRaw('sum(total) as sum1, phone')->where('status','>',4)->groupby('phone')->orderby('id','asc')->paginate(10);
    return view('backend.customer.list-customer',['customer'=>$order,'s'=>$search,'agency'=>$agency,'status'=>1]);
  }
}
