...
{   
   ...
   public function LoadFilter(Request $req){
       ...
       //_________________ END FILTER IN GROUP ATTRIBUTE __________________
        ...
        $list_filter = array();
        $filter = array();
        $html = view('frontend.filter.load-filter-cate',['filter'=>$filter,'filter_0'=>$filter_0,'filter_y'=>$filter_y,'list_filter'=>$list_filter,'frame_list'=>$frame_list,'group_name'=>$group_name]);
    ...
    public function ClickFilter(Request $req){
        ...
        $filter_y = Filter::where('type',1)->whereIn('id',$in_filter)->orderby('name','desc')->orderby('min','asc')->get();
        $filter = array();
        $html = view('frontend.filter.load-filter-cate',['filter'=>$filter,'filter_0'=>$filter_0,'filter_y'=>$filter_y,'list_filter'=>$list_filter,'frame_list'=>array(),'group_name'=>$group_name]);
        $product_view = view('frontend.filter.load-filter-list-frame',array('frames'=>$list_san_pham,'group_attr'=>$group_cate));
        $product_paginate = view('frontend.filter.load-filter-paginate-frame',['frames'=>$list_san_pham,'group_attr'=>$group_cate]);
    ...
    public function LoadFilterSearch(Request $req){
      ...
      $list_filter = array();
      $group_name = array();
      $html = view('frontend.filter.load-filter-cate',['filter_0'=>$filter_0,'filter_y'=>$filter_y,'list_filter'=>$list_filter,'group_name'=>$group_name]);
      return json_encode(array('status'=>true,'html'=>$html.""));
    }
    ...
    public function load_filter_click_search(Request $req){
        ...
        $filter_y = Filter::where('type',1)->whereIn('id',$in_filter)->orderby('name','desc')->orderby('min','asc')->get();
        $filter = array();
        $html = view('frontend.filter.load-filter-cate',['filter'=>$filter,'filter_0'=>$filter_0,'filter_y'=>$filter_y,'list_filter'=>$list_filter,'group_name'=>$group_name]);
        $product_view = view('frontend.filter.load-filter-click-search',array('frames'=>$list_san_pham,'search_frames'=>$a));
        $product_paginate = view('frontend.filter.load-filter-click-paginate-search',['frames'=>$list_san_pham,'status'=>1,'search_frames'=>$a]);
        return json_encode(array('status'=>true,'view'=>$html.'','product_view'=>$product_view.'','product_paginate'=>$product_paginate.''));
    }
