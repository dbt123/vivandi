...
<div class="navbar">
     <div style="float:left;" class="title_form">
          @if(session('admin')->can(['them_san_pham_dai_ly']))   
            <a class="p-themmoi1" id="d_add_agency_frame" style="margin-left:0px;cursor:pointer">+ Thêm sản phẩm</a>
          @else
 ...
 <div class="app-body" id="view">
    <div class="padding">
      <div class="box">
        <div class="box-body">
          <form method="get" action="{!! route('search.list.frame.agency',['id'=>$agency->id,'slug'=>$agency->add_1]) !!}" style="display: inline-block; width: 100%;">
          ...
          <table class="table m-b-none" ui-jp="footable" data-filter="#filter" data-page-size="10">
            ...
            @foreach($frame as $item)
              <tr>
                  ...
                  <td id="d_edit_sku" data-id="{!! $item->id !!}"><span class="d-background"  style="padding: 2px; border-radius: 3px;padding-left: 5px; padding-right: 5px;" >{!! $item->sku_frame_agency !!}</span></td>
                  <td><span style="color:#009D17;" >
                  ...
                  <td>@if($item->label == 0)
                    ...
                    @endif</td>
                  <td><a><span @if(session('admin')->can(['xoa_san_pham_dai_ly'])) class="close-slide del-frame" data-id="{!! $item->id !!}" data-agency="{!! $agency->id !!}" @endif style="font-size:14px;"> × </span></a></td>
              </tr>
            @endforeach  
            </tbody>
...
<div id="m-a-tab-agency" class="modal " data-backdrop="true" style="border:0px;">
  ...
  {{-- popup số lượng --}}
<div id="so-luong-sp_2" class="modal fade animate" data-backdrop="true" >
   <div class="modal-dialog" id="animate">
       <div class="modal-content" style="width:450px" >
            <div  id="d-html">
                <form id="d_edit_from_frame_agency_sku">
                ...
                <input type="hidden" name="frame_id" value="">
                  <div class="modal-header">
                   <table>
                       <tr>
                           ...
                           <td style="float:right;margin-top: 8px;"><button>Nhập</button></td>
                       </tr>
                   </table>
                  </div>
                  <div class="modal-body" style="width:100%">
                    <div class="so-sp">
                       ...
                       <input style="width:100%!important" type="number" name="sku" autocomplete="off" min="1" >
                    </div>
                    <div id="d_errors" style="padding:10px 0px;display:none;color:#DD4E42;"></div>
                  </div>
                </form>
            </div>
       </div>
  ...
  <script>
    ...
    $(document).on('keyup','#d_frame_agency_search',function(e){
      ...
      id = {!! $agency->id !!};
        ...
        type:"post",
        url:"{{route('ajax.search.frame.agency')}}",
        data:{'id':id,'val':val},
    ...
    $(document).on('click','.del-frame',function(e){
      e.preventDefault();
      if(xacnhanxoa('Bạn có chắc muốn xóa sản phẩm thuộc đại lý này không?') === false){
        ...
        $.ajax({
          ...
          type:"post",
          url:"{{route('del.frame.agency')}}",
          data:{'id':id,'agency':agency},
          success:function(data){
            if(data == true){
              window.location.reload();
            }else{
    ...
    spam1 = 0;
    $(document).on('click','#d_edit_sku',function(e){
      e.preventDefault();
      id = $(this).data('id');
      $('#so-luong-sp_2').modal('show');
      $(document).on('submit','#d_edit_from_frame_agency_sku',function(e){
        e.preventDefault();
        var form = $('#d_edit_from_frame_agency_sku')[0];
        var formData = new FormData(form);
        formData.append('id', id);
        formData.append('id_agency', {!! $agency->id !!});
        if(spam1 == 0){
          spam1++;
          $.ajax({
            headers: {
            ...
            type:"post",
            url:"{{route('ajax.edit.frame.agency.sku')}}",
            data: formData,
            contentType: false,
            processData: false,
            success:function(data){
              ...
              if(data.status == true){
                ...
                $('#d_errors').css('display','block').text(data.message).delay(4000).slideUp();
              }
            },
            dataType:"json"
          });
