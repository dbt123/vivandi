              ...
              <tr class="d_width">
                ...
                <td>
                  {{$sku}} sản phẩm
                   ...
                   <?php
                  $order = App\Order_agency::where('agency_id',$item->id)->get();
                  $id = array();
                  if (sizeof($order)>0){
                    foreach ($order as $key => $value) {
                        array_push($id,$value->order_id);
                    }
                    $order_h = App\Order::whereIn('id',$id)->where('status','>',1)->get();
                    $order_d = App\Order::whereIn('id',$id)->where('status',5)->get();
                    $item_id = array();
                    $total = 0;
                    if(sizeof($order_d)>0){
