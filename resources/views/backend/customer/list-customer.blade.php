        ...
        <div class="box-body">
         @include('backend.partials._messages')
          ...
          <form  method="get" action="{{route('search.customer')}}" style="display: inline-block; width: 100%;">
              ...
              <button class="number-post" style="float: right;">{!! sizeof($customer) !!} KH</button>
          </form>
          ...
          <div class="dropdown-menu pull-right" style="padding-left:20px">
                <a  class="dropdown-item" href="{{route('list.customer')}}">Admin</a>
                 <p style="margin-bottom:5px;margin-top:15px">Đại lý</p>
                 <?php
                    $agency_order = App\Agency::get();
                 ?>
                 @foreach ($agency_order as $ao) 
                  <a  class="dropdown-item" href="{{route('agency.customer', ['ag'=>$ao->id])}}">{{$ao->name_agency}}</a>
                 @endforeach
          </div>
        ...
        <style type="text/css">
          ...
          <table class="table m-b-none" ui-jp="footable" data-filter="#filter" data-page-size="10">
            ...
            @foreach($customer as $item)
              @if($status == 0)
            <?php 
                $o_a = App\Order_agency::leftjoin('orders','order_agencys.order_id','=','orders.id')->get();
                $in = array();
                foreach ($o_a as $key => $value) {
                  array_push($in,$value->order_id);
                }
                $order = App\Order::whereNotIn('id',$in)->where('phone',$item->phone)->where('status','>',4)->get();
                $od = $order[sizeof($order)-1];
            ?>    
            @else
              <?php
                $o_a = App\Order_agency::leftjoin('orders','order_agencys.order_id','=','orders.id')->get();
                ...
                $order = App\Order::whereIn('id',$in)->where('phone',$item->phone)->where('status','>',4)->get();
                $od = $order[sizeof($order)-1];
              ?>
            @endif  
            <tr>
              ...
              <td>{!! $item->phone !!}</td>
              <td>{!! $od->email !!}</td>
              <td>@if($status == 0) ADMIN @else {!! $agency->name_agency !!} @endif</td>
               <td><a id="d-list-order" @if($status == 0) data-id="{!! $item->phone !!}" @else data-id="{!! $item->phone !!}" data-agency="{!! $agency->id !!}"  @endif>Danh Sách</a></td>
               <td>{!! $item->total !!}</td>
               <td>{!! number_format((int)$item->sum1,0,'','.') !!} vnđ </td>
            </tr>
            @endforeach
            </tbody> 
                        ...
                        @for ($i = 1; $i <= $customer->lastPage(); $i++)
                          ...
                          @if ($from < $i && $i < $to)
                              <li class="{{ ($customer->currentPage() == $i) ? ' active' : '' }}">
                                  ...
                                  @else
                                    <a  class="{{ ($customer->currentPage() == $i) ? ' active' : '' }}" href="{{$customer->url($i)}}&cus={!! $s !!}" data-page={{$i}}>{{ $i }}</a>
                                  @endif
                              </li>
                          @endif
                        @endfor
...
<div id="m-a-tab" class="modal fade animate" data-backdrop="true" style="border:0px;">
  ...
  {{-- popup --}}
    <div id="popup-quantri-config-tich-diem" class="modal fade animate in" data-backdrop="true" style="display: none;">
       <div class="modal-dialog" id="animate">
           <div class="modal-content">
               <div class="d-ajax-list">
  ...
  <script>
     ...
     @else
      $(document).on('click','#d-list-order',function(e){
        ...
        type:"post",
        url:"{{route('ajax.list.customer.order.agency')}}",
        data:{'phone':phone,'agency':agency},
        success:function(data){
          console.log(data);
          $('#l-box1').html(data.html);
        },
