                                ...
                                <tr>
                                  ...
                                  <td data-type="1">
                                    ...
                                    ?>
                                     ...
                                     $min =  DB::table('frames')->min('price');
                                     $max =  DB::table('frames')->max('price');
                                     if(!$min) $min = 0;
                                     ?>

                                      <div class="itemfilter" style="font-family:'Roboto Bold'">
                                       {{ number_format( (int)$min,0,'','.') }} - {{number_format( (int)$max,0,'','.')}}
                                      </div>

                                      @foreach($list_filter_1 as $key2 => $value2)
                                        <div  class="itemfilter itemfilter_type_1" data-filter="{{$value2->id}}" ><span id="d-price-edit" style="cursor:pointer;">{{ $value2->config_name }}</span>
  ...
  list_c = null;
    ...
    @endif
     ...
     $(document).on('click','.itemfilter_type_0 .x-item,.itemfilter_type_1 .x-item',function(e){
        id = $(this).parent().attr('data-filter');
        container = this;
        if(xacnhanxoa('Bạn có chắc muốn xóa Bộ lọc này không?')===false){
