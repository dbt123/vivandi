                    ...
                    <div class="content-right-model">
                        ...
                        @if(sizeof($group_attr0))
                            @foreach($group_attr0 as $key => $item) 
                                <a href="{!! route('get.Categorydomain',['id'=>$item->id,'slug'=>$item->slug]) !!}"><h5 style="    text-transform: uppercase;"  @if($group_attr->id == $item->id) class="content-right-model-active" @endif >{!! $item->name !!}</h5></a>
                            @endforeach
                        @endif
                    </div>
...
<script>
    ...
    $.ajax({
        ...
        type:"post",
        url:"{{route('load.filter.frontenddomain')}}",
    ...
    $(document).on('click','.d_cate',function(e){
        ...
        id_group = $(this).data('group');
            ...
            type:"post",
            url:"{{route('ajax.pagination.cate.domain')}}",
    ...
    $(document).on('click','.p_pro',function(e){
        ...
        pro = $(this).data('pro');
         $.ajax({
           ...
           type:"get",
           url:"{{route('ajax.pro.cate.domain')}}",
    ...
    $(document).on('click','.filter_click',function(e){
        ...
        $.ajax({
            ...
            type:"post",
            url:"{{route('load.filter.click.frontenddomain')}}",
